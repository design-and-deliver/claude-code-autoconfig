...
**Wrap content in markers** so `/sync-claude-md` knows what's auto-generated. Include an ISO timestamp for the guard hook:
...
**After writing CLAUDE.md**, immediately save a snapshot copy:

```
Write the exact same content to .claude/.claude-md-snapshot
```

This snapshot is used by the guard hook to restore CLAUDE.md if a user edits it directly.
