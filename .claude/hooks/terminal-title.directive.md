...
- {scope}: the DESIGN SCOPE -- the feature area / subsystem under discussion (the WHERE; it
  ...
  NEVER a PHASE OF WORK. A scope names a PLACE, so it must survive the question "where in
  the product?". Banned outright as {scope}: "post-merge", "follow-ups", "cleanup",
  "refactor", "investigation", "next steps", "wrap-up", "review" -- each answers WHEN the
  work sits in a sequence, not WHERE it lives. TEST: if the tab is one of six and the user
  cannot match the title to a subsystem without opening it, the scope failed. Rewrite it as
  the files in play (e.g. "Post-merge follow-ups" over welcome-page.tsx modals was really
  "Welcome page dialogs").
- {use-case}: the GOAL at user-goal level -- an INFINITIVE (base-verb) goal phrase (verb +
  ...
  never "and"-join two use cases. "and" in a {use-case} is the tell that you have TWO titles:
  pick the dominant goal and drop the other, rather than joining them ("Decide on abstraction
  and cleanup plan" is two -- ship "Decide the focus-trap approach").
