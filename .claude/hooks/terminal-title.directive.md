...
- {scope} (first segment): the DESIGN SCOPE -- the subject you are working inside: the feature area
  ...
  rarely. Keep the existing scope if the file already has a title; otherwise infer it from the
  project or work, falling back to "{{FOLDER}}".
...
- {scope}: the DESIGN SCOPE -- the subject under discussion: the feature area / subsystem (the
  boundary). This is the WHERE; it changes rarely.
