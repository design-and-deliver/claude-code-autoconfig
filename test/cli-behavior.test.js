 ...
 *   2c. dev-fleet project     — `--bootstrap` upgrade over a project with tokenGuard.devFleet:
 *                             the fleet-synced token-guard files and hook entries survive the
...
// ── Fixture 2c: a dev-fleet project survives the token-guard retraction ──────
console.log();
console.log('upgrade over a dev-fleet project that holds token-guard (--bootstrap):');

const fleet = makeProject('dev-fleet');
cleanups.push(fleet);
writeFile(fleet, 'CLAUDE.md', '# My Project\n\n<!-- AUTO-GENERATED BY /autoconfig -->\n');
writeFile(fleet, '.claude/.autoconfig-version', '1.0.224');
// tokenGuard.devFleet marks one of the dev fleet's own repos: token-guard.js reaches it through
// scripts/sync-hook-fleet.js, never through this installer, so the retraction must not read it
// as an un-gated leftover (it deleted job-agent-extension's copy on 2026-09-10 and left four
// hook entries pointing at a missing file).
writeFile(fleet, '.claude/cca.config.json', JSON.stringify({ tokenGuard: { devFleet: true } }, null, 2));
writeFile(fleet, '.claude/hooks/token-guard.js', '// fleet copy — must survive\n');
writeFile(fleet, '.claude/commands/token-saver-rationale.md', '<!-- fleet copy — must survive -->\n');
writeFile(fleet, '.claude/settings.json', JSON.stringify({
  hooks: { Stop: [{ matcher: '', hooks: [{ type: 'command', command: 'node "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/token-guard.js"' }] }] }
}, null, 2));

const fleetResult = runCli(fleet, ['--bootstrap'], shimDir);
const fleetClaude = path.join(fleet, '.claude');

test('exits 0', () => {
  assert(fleetResult.code === 0, `expected exit 0, got ${fleetResult.code}\n${fleetResult.out}`);
});

test('dev-fleet token-guard files and hook entries survive the upgrade', () => {
  assert(fs.existsSync(path.join(fleetClaude, 'hooks', 'token-guard.js')), 'fleet token-guard.js must survive');
  assert(fs.existsSync(path.join(fleetClaude, 'commands', 'token-saver-rationale.md')), 'fleet token-saver-rationale.md must survive');
  const merged = readJson(path.join(fleetClaude, 'settings.json'));
  assert(hasCommandContaining(merged, 'Stop', 'token-guard.js'), 'fleet token-guard Stop entry must survive');
});
