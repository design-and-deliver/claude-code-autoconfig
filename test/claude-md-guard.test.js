...
test('Parses valid ISO timestamp from header', () => {
  // Test the regex pattern used in the hook
  const content = '<!-- AUTO-GENERATED BY /autoconfig at 2026-01-14T16:30:45Z — Do not edit. -->';
  const match = content.match(/AUTO-GENERATED.*?at\s+(\d{4}-\d{2}-\d{2}T[\d:]+Z)/);
  assertEqual(match !== null, true, 'Should match timestamp pattern');
  assertEqual(match[1], '2026-01-14T16:30:45Z', 'Should extract correct timestamp');
});

test('Handles timestamp with different times', () => {
  const content = '<!-- AUTO-GENERATED BY /autoconfig at 2025-12-31T23:59:59Z — Do not edit. -->';
  const match = content.match(/AUTO-GENERATED.*?at\s+(\d{4}-\d{2}-\d{2}T[\d:]+Z)/);
  assertEqual(match[1], '2025-12-31T23:59:59Z');
});
...
test('Allows system edit (within 3s of timestamp)', () => {
  ...
  const snapshotPath = path.join(testDir, '.claude', '.claude-md-snapshot');
  const content = `<!-- AUTO-GENERATED BY /autoconfig at ${timestamp} — Do not edit. -->\n\n# Project\n`;
  ...
  assertExists(snapshotPath, 'Snapshot should be created');
});
...
test('Detects user edit (> 3s after timestamp)', () => {
  ...
  const timestamp = oldTime.toISOString().replace(/\.\d{3}Z$/, 'Z');
  const claudeMdPath = path.join(testDir2, 'CLAUDE.md');
  const snapshotPath = path.join(testDir2, '.claude', '.claude-md-snapshot');
  const feedbackPath = path.join(testDir2, '.claude', 'feedback', 'FEEDBACK.md');

  const originalContent = `<!-- AUTO-GENERATED BY /autoconfig at ${timestamp} — Do not edit. -->\n\n# Original\n`;
  const userEditedContent = `<!-- AUTO-GENERATED BY /autoconfig at ${timestamp} — Do not edit. -->\n\n# User Changed This\n`;

  // Set up snapshot (original) and edited file
  fs.writeFileSync(snapshotPath, originalContent);
  ...
  // CLAUDE.md should be restored from snapshot
  const restoredContent = fs.readFileSync(claudeMdPath, 'utf8');
  assertIncludes(restoredContent, '# Original', 'Should restore original content');
